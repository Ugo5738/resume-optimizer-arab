...
const JobsQueue: React.FC<JobsQueueProps> = ({ jobs, onSelectJob }) => {
    ...
    return (
        <Card>
            <h2 className="text-lg font-semibold text-slate-100">Optimization Jobs</h2>
            <div className="mt-4 flow-root">
                <ul role="list" className="-my-4 divide-y divide-gray-700">
                    {jobs.map(job => (
                        <li key={job.id} className="py-4">
                            <button
                                onClick={() => job.status === 'complete' && onSelectJob(job.id)}
                                disabled={job.status !== 'complete'}
                                className="flex items-center w-full space-x-4 text-left disabled:cursor-not-allowed group"
                            >
